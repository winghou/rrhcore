...
@Controller
public class GetWhiteKnightInfoController {
	private static final Logger logger = Logger.getLogger(GetValiCodeFromPhone.class);
	
	@Autowired
	GetWhiteKnightInfoService getWhiteKnightInfoService;
	
	@RequestMapping("/getWhiteKnightInfo")
	public @ResponseBody JSONObject getWhiteKnightInfo (@RequestBody JSONObject jo) {
		try {
			RequestTemplate rt = new RequestTemplate(jo);
			JSONObject detail = getWhiteKnightInfoService.getWhiteKnightInfo(rt.getParams());
